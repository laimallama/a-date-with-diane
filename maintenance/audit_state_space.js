...
function instrument(source) {
  const tokens = tokenize(source.script),
    conditions = [],
    edits = [];
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    const open = tokens[i + 1],
      close = tokens[open.mate];
    const fn = source.functions
      .filter((f) => f.start <= tokens[i].start && f.end > close.end)
      .at(-1);
    const id = conditions.length;
    conditions.push({
      id,
      node: fn?.name || "<top-level>",
      expression: source.script.slice(open.end, close.start),
      line: offset + source.script.slice(0, open.start).split("\n").length,
    });
    edits.push(
      { at: open.end, text: `__auditCondition(${id}, (` },
      { at: close.start, text: "))" },
    );
  }
  let script = source.script;
  for (const e of edits.sort((a, b) => b.at - a.at))
    script = script.slice(0, e.at) + e.text + script.slice(e.at);
  return { script, conditions };
}

const transformedSources = new WeakMap();
function loadRuntime(source, observer = () => {}) {
  const nodes = new Map([["box", element()]]),
    storage = new Map();
  const document = {
    readyState: "loading",
    body: element("body"),
  ...
  if (!transformed) {
    ...
    transformedSources.set(source, transformed);
  }
...
function choices(box) {
  return [
    ...box.innerHTML.matchAll(
      /<button class=['"]choice['"] onclick=(?:"go\('([^']+)'\)"|'go\("([^"]+)"\)')>([\s\S]*?)<\/button>/g,
    ),
  ].map((m) => ({ tag: m[1] || m[2], text: m[3].replace(/<[^>]+>/g, "").trim() }));
}
function leaves(gallery) {
  return ["endings", "hiddenScenes"].flatMap((kind) =>
    gallery[kind].flatMap((x) => (x.variants || [x]).map((v) => ({ ...v, kind }))),
  );
}
...
function audit(options = {}) {
  const maxStates = options.maxStates ?? 20000,
    maxPerNode = options.maxPerNode ?? 32;
  const source = readSource(options.lang || "en", !!options.bilingual),
    startTime = Date.now();
  ...
  const c = g.context,
    initial = plain(c.snapshotGame().state);
  const records = [],
    byKey = new Map(),
    counts = new Map(),
    nodeWitnesses = new Map(),
    edgeWitnesses = new Map();
  const galleryNodes = new Set(),
    galleryEdges = new Set(),
    errors = [],
    findings = new Map(),
    ranges = {};
  const prunedByNode = {},
    missingSnapshotVars = new Map();
  ...
  const scalarGlobals = () =>
    Object.fromEntries(
      Object.keys(c)
        .filter(
          (k) =>
            ["number", "boolean", "string"].includes(typeof c[k]) &&
            !tracked.has(k) &&
            !ignoredGlobals.has(k),
        )
        .map((k) => [k, c[k]]),
    );
  ...
  function route(ref) {
    const out = [];
    for (let id = ref; id !== null && id !== undefined; id = records[id].parent)
      out.push(records[id].tag);
    return out.reverse();
  }
  function witnessRoute(w) {
    return [...route(w.parent), w.tag];
  }
  function note(key, data) {
    if (!findings.has(key)) findings.set(key, { ...data, witness: { ...witness } });
  }
  ...
  function capture(parent, tag, isGallery = false, force = false) {
    const state = plain(c.snapshotGame().state),
      extra = scalarGlobals(),
      available = choices(g.box);
    ...
    for (const [key, value] of Object.entries(state)) {
      if (typeof value === "number") {
        if (!Number.isFinite(value))
          note(`nonfinite:${key}:${tag}`, {
            type: "nonfinite-state",
            node: tag,
            variable: key,
            value: String(value),
          });
    ...
    for (const [key, value] of Object.entries(extra))
      if (value !== initialExtra[key] && !missingSnapshotVars.has(key)) {
        missingSnapshotVars.set(key, { variable: key, value, witness: { ...witness } });
      }
    for (const key of ["pounds", "points", "luckshots", "proc", "blad"])
      if (state[key] < 0)
        note(`negative:${key}:${tag}`, {
          type: "negative-state",
          node: tag,
          variable: key,
          value: state[key],
        });
    if (state.luckshots > 3)
      note(`luckshots:${tag}`, { type: "luckshot-overflow", node: tag, value: state.luckshots });
    ...
    const duplicate = available.filter(
      (v, i) => available.findIndex((x) => x.tag === v.tag && x.text === v.text) !== i,
    );
    if (duplicate.length)
      note(`duplicate:${tag}`, { type: "duplicate-visible-choice", node: tag, choices: duplicate });
    ...
    records.push({ parent, tag, state, extra, available });
    byKey.set(key, id);
  ...
  function step(parent, tag, isGallery = false, force = false) {
    ...
    witness = { parent, tag };
    if (prev && !prev.available.some((x) => x.tag === tag))
      throw Error(`Unreachable requested choice ${prev.tag} -> ${tag}`);
    try {
      c.go(tag);
      return capture(parent, tag, isGallery, force);
    } catch (error) {
      errors.push({ node: tag, message: error.stack, witness: { ...witness } });
      return null;
    }
  }
  ...
  for (const supplied of options.seedPaths || []) {
    if (!Array.isArray(supplied) || !supplied.every((tag) => typeof tag === "string"))
      throw Error("Seed paths must be arrays of choice tags");
  ...
  for (; expanded < records.length && expanded < maxStates; expanded++) {
    ...
    if (options.onProgress && expanded % 1000 === 0)
      options.onProgress({ expanded, retained: records.length, nodes: nodeWitnesses.size });
  }
  const sourceNodes = new Set(source.calls.map((x) => x.source.node)),
    targetNodes = new Set(source.calls.filter((x) => x.kind === "choice").map((x) => x.tag));
  const directEdges = [
    ...new Set(
      source.calls.filter((x) => x.kind === "choice").map((x) => `${x.source.node}->${x.tag}`),
    ),
  ];
  const conditionResults = g.conditions
    .filter(
      (x) =>
        sourceNodes.has(x.node) ||
        /^(?:digestionTick|pregameCatchupTick|afterpee|adjpoints|spendLuckshot|capEndgameLuckshots)$/.test(
          x.node,
        ),
    )
    .map((x) => ({
      ...x,
      outcomes: [false, true].filter((v) => outcomeWitnesses.has(`${x.id}:${v}`)),
    }));
  const report = {
    formatVersion: 1,
    source: source.file,
    sourceSha256: hash(source.html),
    generatedAt: new Date().toISOString(),
    method:
      "Actual choice-driven navigation; all Gallery seeds plus breadth-first alternative exploration; exact full snapshot/scalar-global fingerprints retained under explicit budgets.",
    limits: {
      maxStates,
      maxPerNode,
      seedStatesExemptFromBudget: true,
    ...
    summary: {
      sourceFunctions: source.functions.length,
      textFunctions: sourceNodes.size,
      sourceChoiceCalls: source.calls.filter((x) => x.kind === "choice").length,
      staticChoiceTargets: targetNodes.size,
      staticDirectEdges: directEdges.length,
      galleryRoutes: book.length,
      additionalSeedPaths: (options.seedPaths || []).length,
      galleryNodes: galleryNodes.size,
      galleryEdges: galleryEdges.size,
      retainedExactStates: records.length,
      seedStates,
      expandedStates: expanded,
      observedNodes: nodeWitnesses.size,
      observedEdges: edgeWitnesses.size,
      ...
      conditionSites: conditionResults.length,
      conditionSitesObserved: conditionResults.filter((x) => x.outcomes.length).length,
      ...
      findingKindsAtNodes: findings.size,
      elapsedSeconds: (Date.now() - startTime) / 1000,
    },
    ranges,
    statesPerNode: Object.fromEntries([...counts].sort()),
    prunedByNode,
    ...
    conditions: conditionResults.map((x) => ({
      ...x,
      witnesses: Object.fromEntries(
        x.outcomes.map((v) => [String(v), witnessRoute(outcomeWitnesses.get(`${x.id}:${v}`))]),
      ),
    })),
    findings: [...findings.values()].map(({ witness: w, ...rest }) => ({
      ...rest,
      path: witnessRoute(w),
    })),
    errors: errors.map(({ witness: w, ...rest }) => ({ ...rest, path: witnessRoute(w) })),
    mutableScalarsOutsideSnapshot: [...missingSnapshotVars.values()].map(
      ({ witness: w, ...rest }) => ({ ...rest, path: witnessRoute(w) }),
    ),
    ...
    sourceChoiceSites: source.calls
      .filter((x) => x.kind === "choice")
      .map((x) => ({ ...x.source, tag: x.tag, text: x.text })),
  };
  return report;
}
...
if (require.main === module) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    else if (arg === "--seed-paths")
      options.seedPaths = JSON.parse(fs.readFileSync(path.resolve(args[++i]), "utf8"));
    else if (arg === "--bilingual") options.bilingual = true;
    else if (arg === "--no-gallery-seeds") options.seedGallery = false;
    else if (arg === "--progress")
      options.onProgress = (x) => process.stderr.write(JSON.stringify(x) + "\n");
    else throw Error("Unknown argument: " + arg);
  }
  if (options.seedPaths !== undefined && !Array.isArray(options.seedPaths))
    throw Error("Seed paths file must contain an array of paths");
  for (const name of ["maxStates", "maxPerNode"])
