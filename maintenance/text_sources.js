...
function tokenize(source) {
  ...
  while (i < source.length) {
    ...
    } else if (ch === '/' && (!tokens.length || /^(?:[=(,:;!&|?{}\[]|return|case|=>)$/.test(tokens.at(-1).raw))) {
      type = 'regexp'; i++;
      let inClass = false;
      while (i < source.length) {
  ...
  tokens.forEach((t, index) => {
    ...
    if (['(', '[', '{'].includes(t.raw)) stack.push(index);
      ...
      if (opening === undefined || tokens[opening].raw !== close[t.raw]) throw Error('Unbalanced source tokens');
...
function argumentsAt(tokens, opening) {
  ...
  for (let i = start; i < tokens[opening].mate; i++) {
    if (tokens[i].raw === ',' && tokens[i].type === 'punctuation') { args.push(tokens.slice(start, i)); start = i + 1; }
...
function readSource(lang, bilingual = false) {
  ...
  const tokens = tokenize(script), functions = [], calls = [], variants = [], dictionary = new Map();
  for (let i = 0; i < tokens.length; i++) {
    if (tokens[i].raw === 'function' && tokens[i + 1]?.type === 'identifier' && tokens[i + 2]?.raw === '(') {
      const body = tokens[i + 2].mate + 1;
      if (tokens[body].raw !== '{') throw Error('Unsupported function syntax');
      functions.push({ name: tokens[i + 1].raw, start: tokens[i].start, end: tokens[tokens[body].mate].end });
    }
  }
  const slots = new Map();
  function argument(tokens) {
    if (!tokens?.length) return null;
    return { text: tokens[0].type === 'string' ? tokens[0].value : null, static: tokens.length === 1 && tokens[0].type === 'string', expression: script.slice(tokens[0].start, tokens.at(-1).end) };
  }
  for (let i = 0; i < tokens.length; i++) {
    const t = tokens[i];
    if (t.type === 'identifier' && ['s', 'c', 'sAlt', 'cAlt'].includes(t.raw) && tokens[i + 1]?.raw === '(' && !['function', '.'].includes(tokens[i - 1]?.raw)) {
      const fn = functions.filter(f => f.start < t.start && f.end > t.end).at(-1);
      if (!fn) throw Error('Text call outside a named function');
      const args = argumentsAt(tokens, i + 1), kind = t.raw.startsWith('s') ? 'story' : 'choice';
      const arg = argument(args[kind === 'story' ? 0 : 1]);
      if (!arg || arg.text === null) throw Error('Unsupported text expression in ' + fn.name);
      const slot = (slots.get(fn.name) || 0) + 1;
      slots.set(fn.name, slot);
      calls.push({ kind, source: { node: fn.name, slot }, tag: kind === 'choice' ? argument(args[0]).text : undefined, ...arg, alternate: argument(args[kind === 'story' ? 1 : 2]) });
    }
    if (t.raw === 'alternateTranslations' && tokens[i + 1]?.raw === '=' && tokens[i + 2]?.raw === '{') {
      const end = tokens[i + 2].mate;
      for (let j = i + 3; j < end;) {
        const key = tokens[j++], colon = tokens[j++], value = tokens[j++];
        if (key.type !== 'string' || colon.raw !== ':' || value.type !== 'string') throw Error('Unsupported translation dictionary property');
        if (dictionary.has(key.value)) throw Error('Duplicate translation key in ' + file + ': ' + key.value);
        dictionary.set(key.value, value.value);
        if (j < end && tokens[j++].raw !== ',') throw Error('Unsupported translation dictionary separator');
      }
    }
    if (/^notYet(?:Sitting|Standing|Queue)$/.test(t.raw) && tokens[i + 1]?.raw === '=' && tokens[i + 2]?.raw === 'new' && tokens[i + 3]?.raw === 'Array') {
  ...
  return { file, html, script, functions, calls, variants, dictionary };
}
