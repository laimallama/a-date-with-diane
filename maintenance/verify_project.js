...
function loadGame(file) {
  ...
  for (const [, attrs, script] of blocks)
    if (!/application\/(?:ld\+)?json/.test(attrs)) new vm.Script(script, { filename: file });
  ...
  assert.match(
    html,
    /\.gallery-row\s*\{[\s\S]*?display:\s*grid/,
    "Gallery rows must stack as a grid, not wrap as chips",
  );
...
function choiceTags(box) {
  return [
    ...box.innerHTML.matchAll(
      /<button class=['"]choice['"] onclick=(?:"go\('([^']+)'\)"|'go\("([^"]+)"\)')>/g,
    ),
  ].map((m) => m[1] || m[2]);
}
function snapshot(g) {
...
function guide(g) {
  return JSON.stringify([g.context.guideIndex, g.context.guideActive]);
}
function plain(x) {
...
const referenceStates = new Map();
{
  for (const command of [
    ...
    execFileSync(
      process.execPath,
      [path.join(ROOT, "maintenance", command[0]), ...command.slice(1)],
      { stdio: "inherit" },
    );
}
...
for (const lang of langs) {
  ...
  for (const edition of editions) {
    ...
    const filename = `dianedate_${lang}${bilingual ? "_bilingual" : ""}.html`;
    ...
    if (!bilingual)
      assert.deepEqual(
        plain(initial.gallery),
        gallerySnapshot[lang],
        "Embedded Gallery must match JSON",
      );
    let steps = 0;
    for (const leaf of book) {
      const g = loadGame(file),
        c = g.context;
      c.guideTags = leaf.tags;
      c.guideIndex = 0;
      c.guideActive = true;
      c.guideOn = true;
      ...
      if (lang === "en" && edition === "standalone") referenceStates.set(leaf.id, digest);
        ...
        assert.equal(
          digest,
          referenceStates.get(leaf.id),
          `State differs from English: ${path.basename(file)} / ${leaf.id}`,
        );
      ...
      const skipped = loadGame(file),
        walked = loadGame(file);
      Object.assign(skipped.context, {
        guideTags: leaf.tags,
        guideIndex: 0,
        guideActive: true,
        guideOn: true,
      ...
      assert.deepEqual(
        plain(skipped.context.snapshotGame().state),
        plain(walked.context.snapshotGame().state),
        "Skip state mismatch",
      );
      assert.equal(
        skipped.context.gameHistory.length,
        walked.context.gameHistory.length,
        "Skip must record every history step",
      );
      skipped.context.goback();
      walked.context.goback();
      assert.deepEqual(
        plain(skipped.context.snapshotGame().state),
        plain(walked.context.snapshotGame().state),
        "Back after Skip state mismatch",
      );
      // Skip is intentionally single-use; its toolbar button may differ.
      assert.equal(
        skipped.box.innerHTML.replace(/<div class='nav-row'>[\s\S]*$/, ""),
        walked.box.innerHTML.replace(/<div class='nav-row'>[\s\S]*$/, ""),
        "Back after Skip story mismatch",
      );
    }
    totalSteps += steps;
    totalRoutes += book.length;
    console.log(
      `OK ${path.relative(ROOT, file)}: ${book.length} routes, ${steps} Back/replay checks, Skip and state parity`,
    );
  }
}
...
console.log(
  `PASS: ${totalRoutes} route/edition combinations; ${totalSteps} Back/replay checks. No files written.`,
);
